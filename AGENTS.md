...
- 视觉系统使用冷灰画布与表面、钴蓝操作色、玉绿引用/成功色和琥珀等待/警告色；统一使用 `background`、`surface-raised`、`primary`、`success`、`warning`、`info` 等语义 token，不以具体色名创建业务变量。
- 页面采用连续平面、留白和细分隔线组织。卡片只用于重复对象、弹窗和独立工具，不将页面分区做成卡片，也不创建嵌套卡片。
- “折页与引用轨道”只用于品牌标记、选中资料、引用来源和答案定位；不得扩散为普通页面装饰。全局导航与 Tabs 的选中态使用背景、文字和克制阴影表达，不增加装饰性伪元素。禁止渐变、装饰光斑、大面积蓝紫背景和营销式大标题。
- 排版以 12px 元数据、13px 紧凑控件、14px 正文、16px 分区标题和 22px 页面标题为基准；字距统一为 `0`，基础圆角为 6px，弹窗不超过 8px，阴影仅用于浮层。
...
- 面板展开、选择切换和引用定位动效控制在 140–180ms；不得用持续动效表达静态状态。
...
- 全局侧栏使用冷灰半透明表面，选中项可以使用低透明操作色、轻量 backdrop blur 和多层低透明阴影营造玻璃质感；不得使用渐变或高饱和实色铺满侧栏。
...
- 复杂功能需要补充上下文时，优先在页面内部增加约 200px 的白色局部来源栏或检查器，不扩展为永久双侧栏；局部栏使用细分隔线与半透明灰色选中项，窄屏下必须收起或转为单区布局。
