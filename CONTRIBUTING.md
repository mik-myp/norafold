...
- 更新检查只能通过 Main/Preload 边界访问 GitHub，不能在 Renderer 中直接请求更新 API 或打开任意外部 URL。
...
项目不配置 push/PR 的长期 GitHub CI；发布标签会在 Release 工作流中重新执行上述验证。

影响 Forge maker、Fuses、原生依赖、更新检查或发布流程时，还必须运行 `vp run make`。提交 Hook 会先格式化暂存文件，再执行格式和代码检查；不要跳过 Hook 来隐藏失败。

## Pull Request

PR 描述应包含变更目的、用户可见行为、验证命令和已知限制。安全边界、依赖 override、构建链、更新策略或架构方向发生变化时，应新增或更新 `docs/adr` 下的架构决策记录，并同步更新 `CHANGELOG.md` 的 `Unreleased` 部分。
