...
- 增加主题、颜色预设、字体、圆角、密度、侧边栏、布局、内容宽度和界面方向设置。
...
- 设置导航改为 new-api 风格的侧栏下钻视图，并修正主题内容宽度与各配置项的交互一致性。
- 主题设置迁移至设置页“界面语言”下方的右侧抽屉入口，并直接采用 new-api 的颜色预设、配置顺序、预览网格和重置交互。
- 统一应用为“折页研究台”视觉系统，并参考 Dify 的玻璃侧栏、连续主表面和局部导航层级重构工作台、知识库、设置与错误页面。
- 修复 Windows 与 Linux 安装包中 `sqlite-vec` 原生扩展的外部资源路径。
